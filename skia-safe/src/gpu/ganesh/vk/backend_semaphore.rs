...
pub mod backend_semaphores {
    ...
    /// or [`crate::Surface::wait`] operation. The caller must keep the
    /// semaphore alive until all such operations complete.
    pub fn make_vk(semaphore: vk::Semaphore) -> BackendSemaphore {
    ...
    pub fn get_vk_semaphore(semaphore: &BackendSemaphore) -> Option<vk::Semaphore> {
        ...
        let handle = unsafe { sb::C_GrBackendSemaphores_GetVkSemaphore(semaphore.native()) };
        if handle.is_null() {
            None
        } else {
            Some(handle)
